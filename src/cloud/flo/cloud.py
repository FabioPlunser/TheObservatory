    ...
    async def process_image(self, camera_id: int, image_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'Contents' not in known_faces:
                ...
                return {"status": "error", "message": "No known faces available for comparison"}

            for known_face in known_faces.get('Contents', []):
                logger.debug(f"Comparing against known face: {known_face['Key']}")

                response = self.rekognition_client.compare_faces(
                    SourceImage={'S3Object': {'Bucket': KNOWN_FACES_BUCKET, 'Name': known_face['Key']}},
                    TargetImage={'S3Object': {'Bucket': FACES_TO_CHECK_BUCKET, 'Name': image_key}},
    ...
    def log_detection(self, camera_id: int, image_key: str, is_known: bool) -> None:
        ...
        try:
            self.cursor.execute(
                ...
                (camera_id, image_key, int(is_known), datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            )
            self.db_conn.commit()
...
class NATS:
    ...
    async def send_alarm_message(self, camera_id: int, presigned_image_url: str) -> None:
