...
class SimulatedCamera:
    ...
    def get_ffmpeg_input_args(self, video_paths: List[str]) -> tuple[List[str], str]:
        """Get FFmpeg input arguments using a different approach for different platforms.
        ...
        # On macOS, use a temporary file instead of pipe
        if platform.system() == "Darwin":
            ...
            return [
                ...
                "-1",
                "-i",
                temp_file.name,  # Use the temporary file instead of pipe
                "-filter:v",
                f"setpts=PTS/{self.frame_rate}/TB",
            ], ""
        else:
            # For other platforms, continue using pipe
            return [
                "-fflags",
                "nobuffer",
                "-flags",
    ...
    async def _handle_ffmpeg_process(
        self, process, concat_content: str, stop_event: Optional[asyncio.Event]
    ):
    ...
    async def start_streaming(
        self, video_infos: List[VideoInfo], stop_event: Optional[asyncio.Event] = None
    ):
        """Start streaming videos using in-memory concat list."""
        if not video_infos or not all(
            Path(info.full_path).exists() for info in video_infos
        ):
...
async def run_camera(
    ...
    try:
        if (
            not await camera.discover_edge_server()
            or not await camera.register_with_edge()
        ):
            return
